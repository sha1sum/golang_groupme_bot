...
func (handler Handler) Handle(term string, c chan *bot.OutgoingMessage, message bot.IncomingMessage) {
	...
	if len(items) < 1 {
		c <- &bot.OutgoingMessage{Err: errors.New("No results for \"" + term + "\".")}
		return
	}
...
func parseLink(item matchers.Item) *bot.OutgoingMessage {
	...
	if err != nil {
		return &bot.OutgoingMessage{Err: err}
	}
	...
	ls := queryVals["url"]
	if len(ls) < 1 {
		return &bot.OutgoingMessage{Message: item.Link}
	}
	fmt.Println("Found link", ls[0])
	return &bot.OutgoingMessage{Message: ls[0]}
}
